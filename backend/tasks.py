...
def do_import(self, url, user_id=None):
    ...
    Args:
        ...
        url: URL-адрес файла для импорта
    ...
    try:
        ...
        try:
            ...
            raise ValueError("Пользователь не найден либо не является магазином")

        try:
            response = requests.get(url, timeout=5)
            response.raise_for_status()
        except RequestException as e:
            return JsonResponse({'status': False, 'Ошибка импорта': str(e)})

        data_request = yaml.safe_load(response.content)
        ...
        # Создание либо обновление магазина
        shop, created = Shop.objects.get_or_create(name=data_request['shop'], defaults={'user': user})
        if not created and shop.user != user:
            raise PermissionError("Этот магазин принадлежит другому пользователю")

        # Обработка категорий
        for category_data in data_request['categories']:
            Category.objects.get_or_create(
                id=category_data['id'],
                defaults={'name': category_data['name']}
            )

        # контекстный менеджер транзакции загрузки товаров
        with transaction.atomic():
            ProductInfo.objects.filter(shop=shop).delete()

            for item in data_request['goods']:
                ...
                # Получение категории как объекта
